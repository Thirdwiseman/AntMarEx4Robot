...
public class Junction4 implements Behavior {
	...
	public Junction4(NXTRegulatedMotor right, NXTRegulatedMotor left,
	...
	private boolean isWall() {
		...
		if (this.getRange() < 15) {
			...
			left.backward();
			right.backward();
			while (!(Math.abs(SensorPort.S2.readValue() - this.blackLeft) <= 3 && Math
					.abs(SensorPort.S4.readValue() - this.blackRight) <= 3)) {
			}
			right.stop();
			left.stop();
			left.rotate(40);
